...
if (!file.exists("core_src.zip"))
   system(paste0("cp ", system.file("core_src.zip", package="BiocBBSpack"), " ", td), intern=TRUE)
...
We'll use the `r CRANpkg("rcmdcheck")` package to capture information
...
dfc = dd %>% filter(Package %in% coreset() & dependency %in% coreset()) 
gg = buildPkgDependencyIgraph(dfc)
plot(gg)
```
Some of the packages in the small set are isolated.
```{r lkiso}
setdiff(coreset(), union(dfc$Package, dfc$dependency))
```
...
Here is a screen shot of the `browse_checks` app in BiocBBSpack
